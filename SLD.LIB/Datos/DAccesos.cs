...
namespace SLD.LIB.Datos
{
    public class DAccesos
    {
        ...
        public T EjecutarConsultaSP<T>(string nombreSP, DynamicParameters parametros) where T : class
        {
            ...
            {
                connection.Open();
                return connection.Query<T>(sql: nombreSP,
                                                    param: parametros,
                                                    commandType: CommandType.StoredProcedure).FirstOrDefault();
            }
        }
        public List<T> TrerLista<T>(string nombreSP, DynamicParameters parametros) where T : class
        {
            using (var connection = new SqlConnection(cadenaConexion))
            {
                connection.Open();
        ...
        public List<object> EjecutarConsultaSP(string nombreSP, params Func<GridReader, object>[] entidades)
        {
            return EjecutarConsultaSP(nombreSP, null, entidades);
        }
